...
export const RewardedLeakGate: React.FC<RewardedLeakGateProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md p-4">
      <div className="w-full max-w-sm apple-glass rounded-3xl p-6 border border-amber-500/30 flex flex-col space-y-4 shadow-2xl relative text-break-boundary">
        <button
          onClick={() => {
        ...
        <div className="flex flex-col items-center text-center space-y-2 pt-2">
          <div className="w-12 h-12 rounded-2xl bg-amber-500/20 border border-amber-500/40 flex items-center justify-center text-amber-500 dark:text-amber-300">
            <Lock className="w-6 h-6" />
          </div>
          <div>
            <span className="px-2 py-0.5 rounded bg-amber-500/10 border border-amber-500/30 font-mono text-[9px] uppercase tracking-wider text-amber-600 dark:text-amber-400">
              RESTRICTED INVESTIGATIVE LEAK
            </span>
            <h3 className="font-semibold text-sm text-[var(--text-primary)] tracking-tight mt-1.5 text-break-boundary">
              {news.title}
            </h3>
            <p className="text-xs text-[var(--text-secondary)] mt-1 text-break-boundary">
        ...
        <div className="space-y-2 pt-2">
          ...
          <button
            ...
            className="w-full py-2.5 px-4 rounded-xl bg-[var(--bg-pill)] hover:bg-[var(--bg-pill-hover)] border border-[var(--border-subtle)] text-[var(--text-primary)] font-medium text-xs flex items-center justify-center space-x-1.5 transition-all"
          >
