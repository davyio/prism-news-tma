...
export const StoryQuoteModal: React.FC<StoryQuoteModalProps> = ({
  ...
  const handleShareStory = () => {
    ...
    if (tg && typeof tg.shareToStory === 'function') {
      tg.shareToStory('https://prism-news-tma.vercel.app/icons/prism-monolith.png', {
        ...
        widget_link: {
          url: 'https://t.me/PrismNewsBot',
          name: 'PRISM News',
        }
      });
    } else if (tg && typeof tg.openTelegramLink === 'function') {
      const shareUrl = `https://t.me/share/url?url=${encodeURIComponent('https://t.me/PrismNewsBot')}&text=${encodeURIComponent(quoteText)}`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md animate-fade-in p-4">
      <div className="w-full max-w-sm apple-glass rounded-3xl p-5 border border-white/10 flex flex-col space-y-4 shadow-2xl relative">
        <button
          onClick={() => {
            triggerHaptic('light');
            onClose();
        ...
        <div className="text-center pt-1">
          ...
          <h3 className="font-semibold text-sm text-white tracking-tight">Export to Telegram Stories</h3>
        </div>

        {/* Visual Card Mockup */}
        <div className="w-full rounded-2xl p-5 bg-gradient-to-br from-neutral-900 via-black to-neutral-950 border border-white/15 shadow-xl space-y-3 relative overflow-hidden">
          ...
          <p className="font-bold text-sm text-white leading-snug font-sans">
            &quot;{perspective.headline}&quot;
          </p>

          <p className="text-[11px] text-neutral-300 font-sans leading-relaxed border-l-2 border-white/20 pl-2.5 my-2">
            {perspective.keyTakeaways[0]}
          </p>
        ...
        <div className="space-y-2">
          ...
          <button
            ...
            className="w-full py-2 px-3 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.06] text-neutral-300 font-mono text-[11px] flex items-center justify-center space-x-1.5 transition-all"
          >
